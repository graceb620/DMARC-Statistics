...
x.train <- model.matrix(first_visit_2023 ~ n_people_in_household + snap + snap_first_visit + snap_change_2023 + household_income_median + fed_poverty_level_first + last_homeless_state + first_housing_type + last_housing_type + own_or_buying + more_than_one_change_location + elderly + child + working_age + college_education + single_parent, data = train.df)[,-1]
x.test <- model.matrix(first_visit_2023 ~ n_people_in_household + snap + snap_first_visit + snap_change_2023 + household_income_median + fed_poverty_level_first + last_homeless_state + first_housing_type + last_housing_type + own_or_buying + more_than_one_change_location + elderly + child + working_age + college_education + single_parent, data = test.df)[,-1]
...
lasso_rocCurve <- roc(response = as.factor(test.df.preds$first_visit_2023), predictor = test.df.preds$lasso_pred, levels = c("0", "1"))
ridge_rocCurve <- roc(response = as.factor(test.df.preds$first_visit_2023), predictor = test.df.preds$ridge_pred, levels = c("0", "1"))
