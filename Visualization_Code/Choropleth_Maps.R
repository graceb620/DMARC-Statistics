...
# 
# This document was used for making the Choropleths maps 
...
### --- Making the Datasets ---------------------------------------------------
hh_23 <- read.csv("Data/hh_data23.csv")

hh_new_23 <- hh_23 %>%
  filter(first_visit_2023 == 1) %>% 
  group_by(zip) %>%
  summarize(
    count = n()
    )

hh_returner_23 <- hh_23 %>%
...
# Merge your data
zip_data <- data.frame(
  ZCTA5CE20 = hh_new_23$zip,
  value = hh_new_23$count
)
